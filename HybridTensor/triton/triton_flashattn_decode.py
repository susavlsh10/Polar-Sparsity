...
# from flash_attn.utils import _strides, get_padded_headsize
...
def _fwd_kernel_splitK(
    ...
    N_CTX_NEW,
    BLOCK_N_PER_SPLIT,
    H_q: tl.constexpr,  # number of heads in query
    H_kv: tl.constexpr, # number of heads in key/value
    G_q: tl.constexpr,  # number of groups in query, this results in 1
    ...
    off_g_q = (off_zhg % G_q) # G_q always 1, so off_g_q = 0
    splitk_idx = tl.program_id(2).to(tl.int64)

    # pick batch index
    if USE_CACHE_BATCH_IDX:
        cache_batch_idx = tl.load(Cache_batch_idx + off_z).to(tl.int64)
    else:
    ...
    if USE_CACHE_SEQLENs:
        cache_seqlen_last_idx = tl.load(Cache_seqlens + off_z).to(tl.int32)
        if NEW_KV:
            kv_len = (cache_seqlen_last_idx + N_CTX_NEW).to(tl.int32)
        else:
            kv_len = cache_seqlen_last_idx
    else:
        kv_len = (N_CTX_K).to(tl.int32)
    hi = tl.minimum((splitk_idx + 1) * BLOCK_N_PER_SPLIT, kv_len) #.to(tl.int64)
    ...
    # FIX: make_block_ptr offsets should be int32
    Q_block_ptr = tl.make_block_ptr(
    ...
    for start_n in range(lo, hi, BLOCK_N):
        k, v = load_k_v_group(
            ...
            BOUNDS_CHECKS_N or (BLOCK_DMODEL != ACTUAL_BLOCK_DMODEL),   # <-- here,
            1,
            BLOCK_DMODEL,
            ACTUAL_BLOCK_DMODEL,
            Q.dtype.element_ty,
...
def attention_decode_forward_triton_impl(q, k, v, sm_scale, causal, alibi_slopes,
    ...
    out_splitk = torch.empty([batch_size * num_actual_groups * heads_per_actual_group, splitK_pow2, seqlen_q_ceil, dim_padded], dtype=torch.float32, device=q.device)
    metadata = torch.empty([batch_size * num_actual_groups * heads_per_actual_group, 2, splitK_pow2, seqlen_q_ceil], dtype=torch.float32, device=q.device)

    lse = torch.empty((batch_size * num_actual_groups * heads_per_actual_group, seqlen_q), device=q.device, dtype=torch.float32)
    grid = (triton.cdiv(seqlen_q, BLOCK_M), batch_size * num_actual_groups * heads_per_actual_group, split_k)
    # grid = (triton.cdiv(seqlen_q, BLOCK_M), batch_size * n_group_k * heads_per_group_k, split_k)
    ...
    grid = (batch_size * num_actual_groups * heads_per_actual_group, seqlen_q, k_block_num)
