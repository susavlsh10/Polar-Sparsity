...
def _fwd_kernel_splitK(
    ...
    Seq_len,
    batch_head_index, # {[B0, H1], [B1, H1], ...}, -> [N_selected, 2]; new argument ## if GQA, we select from groups: {[B0, G1], [B1, G1], ...}, -> [N_selected, 2]
    stride_qz,
    stride_qm,
    stride_qg,
    stride_qh,
    ...
    # off_z = off_zhg // (H * G)  # batch
    # off_h = (off_zhg // G) % H  # head
    off_g = off_zhg % G         # group, when G == 1, off_g = 0
    off_idx = off_zhg // G      # when G == 1, off_idx = off_zhg    
    
    # Compute the pointer to the batch_head_index
    batch_head_index_ptr = batch_head_index + off_idx * 2

    # Load off_z and off_h separately
    off_z = tl.load(batch_head_index_ptr + 0, mask=True, other=0).to(tl.int64)
    off_h = tl.load(batch_head_index_ptr + 1, mask=True, other=0).to(tl.int64)
...
def _splitK_reduce(
    ...
    off_idx = off_zhg // G  # when G == 1, off_idx = off_zhg
    off_g = off_zhg % G     # when G == 1, off_g = 0
...
class select_attention(torch.autograd.Function):
    ...
    def forward(cls, q, k, v, scale_float, batch_head_index, seq_len=None):
        ...
        N_selected = batch_head_index.shape[0]
